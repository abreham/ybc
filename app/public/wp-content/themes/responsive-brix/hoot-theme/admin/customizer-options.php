...
function hoot_theme_customizer_options() {
	...
	$lcontent .= '<a class="hoot-cust-link" href="' .
				 'https://demo.wphoot.com/responsive-brix/' .
	...
	$lcontent .= '<a class="hoot-cust-link" href="' .
				 ...
				 '</span></a>';
	$lcontent .= '<a class="hoot-cust-link" href="' .
				 'https://wordpress.org/support/theme/responsive-brix/reviews/?filter=5#new-post' .
				 '" target="_blank"><span class="hoot-cust-link-head">' .
	...
	if ( current_theme_supports( 'hoot-widgetized-template' ) ) :
		...
		$settings['wt_html_slider_min_height'] = array(
			...
			'default'     => 375,
			'description' => __('Leave empty to let the slider height adjust automatically.', 'responsive-brix'),
			'input_attrs' => array(
				'placeholder' => __( '(in pixels)', 'responsive-brix' ),
			),
		);
	...
	if ( current_theme_supports( 'woocommerce' ) ) :
		...
		$sections[ $section ] = array(
			'title'       => __( 'WooCommerce (Responsive Brix)', 'responsive-brix' ),
...
function hoot_theme_add_customizer_options() {
	...
	// 						'url'    => 'https://wordpress.org/support/theme/responsive-brix/reviews/?filter=5#new-post',
	// 						'icon'   => 'fas fa-star' ),
	// 	) );
	...
	$hoot_customizer->add_infobuttons( array(
		'premium' => array( 'text'   => __( 'Premium', 'responsive-brix'),
							'type'   => 'premium',
							'url'    => esc_url( admin_url('themes.php?page=responsive-brix-premium') ),
							'icon'   => 'fa-rocket fas' ),
		) );
